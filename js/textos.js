const translations = {
  es: {
    // HERO - Frases aleatorias
    "hero-title-1": "Envía dinero a Bolivia sin comisiones y con el mejor tipo de cambio",
    "hero-title-2": "Envía dinero como si mandaras un mensaje de texto, ¡así de fácil! ??",
    "hero-title-3": "Con LUPO tu familia recibe más y más rápido. ¡Ahorra tiempo y gana tranquilidad! ??",
    "hero-title-4": "Todo desde tu teléfono, estés donde estés. Simple, seguro y humano.",
    "hero-title-5": "¿Prefieres atención personalizada? Aquí siempre puedes pedirla. ??",
    "hero-title-6": "En LUPO somos más humanos, por eso te damos el mejor tipo de cambio. ??",
    "hero-title-7": "Cada envío es una experiencia agradable y sin complicaciones.",
    "hero-title-8": "Porque cada envío cuenta: aquí tienes atención real en cada paso. ??",
    "hero-title-9": "Más que un envío, es apoyo real para los tuyos. ¡Hazlo fácil, hazlo LUPO!",
    "hero-subtitle": "Con Lupo, tu familia recibe hasta un 20% más en Bolivia comparado con otros servicios. Apoya a los tuyos, sin perder en comisiones.",
    "btn-send": "Enviar ahora",
    "btn-how": "Cómo funciona",

    // CALCULADORA - Frases aleatorias
    "calc-title-1": "¿Cuánto más puede recibir tu familia hoy?",
    "calc-title-2": "Con Lupo, tu dinero vale hasta un 20% más en Bolivia.",
    "calc-title-3": "Envía dinero sin comisiones y apoya más a tu tierra.",
    "calc-title-4": "Haz que tu esfuerzo llegue completo, sin recortes ni descuentos.",
    "calc-title-5": "Apoya a tu familia enviando más, sin pagar comisiones.",
    "calc-title-6": "Elige Lupo y tu familia recibe más, rápido y seguro.",
    "calc-placeholder": "Cantidad en EUR",
    "calc-btn": "Enviar",

    // FRASES DE AHORRO - Frases aleatorias
    "save-1": "Tus envíos con LUPO te dan hasta {{save}} más para tu familia en Bolivia. ??",
    "save-2": "¿Sabías que aquí recibes {{save}} más que usando Western, Ria o MoneyGram? ??",
    "save-3": "Más ganancias, {{save}} más de tranquilidad. Eso solo te lo da LUPO. ??",
    "save-4": "Elige LUPO: aquí tu apoyo suma {{save}} extra para los tuyos.",
    "save-5": "Aquí, cada euro rinde {{save}} más que con la competencia. ??",
    "save-6": "Envía con LUPO y gana {{save}} extra en cada transferencia.",
    "save-7": "Manda dinero sin perder: tu familia recibe {{save}} más solo aquí.",
    "save-8": "Con LUPO, el esfuerzo de tu trabajo significa {{save}} más en casa. ??",

    // BENEFICIOS - Features
    "features-title": "¿Por qué elegir Lupo?",
    "feature-1-title": "Más para tu familia",
    "feature-1-text": "Con Lupo, cada euro que envías rinde hasta un 20% más en Bolivia, comparado con servicios tradicionales.",
    "feature-2-title": "Sin comisiones",
    "feature-2-text": "No cobramos ninguna comisión. Todo lo que mandas, llega completo a los tuyos.",
    "feature-3-title": "Entrega inmediata",
    "feature-3-text": "El dinero llega en minutos a Bolivia, siempre en bolivianos exactos y sin sorpresas.",
    "feature-4-title": "Total seguridad",
    "feature-4-text": "Tus envíos están protegidos, con tecnología avanzada y seguimiento en tiempo real.",
    "feature-5-title": "Atención personalizada",
    "feature-5-text": "Estamos para ayudarte. Personal en Italia y Bolivia, presencial o en línea.",
    "feature-6-title": "Fácil y sin complicaciones",
    "feature-6-text": "Envía desde donde estés, sin papeleos ni trámites difíciles.",

    // COMO FUNCIONA
    "how-title": "¿Cómo funciona LUPO?",
    "step-1-title": "Depositas en Italia",
    "step-1-text": "Puedes depositar en cuenta bancaria o en persona con nuestro equipo, como prefieras.",
    "step-2-title": "Entregamos en Bolivia",
    "step-2-text": "El dinero se entrega en cuenta bancaria o en efectivo, según le convenga a tu familia.",
    "step-3-title": "Seguimiento y comprobantes",
    "step-3-text": "Te avisamos cuando el dinero está listo, con comprobantes y acompañamiento en todo momento.",
    "how-final": "En minutos, tu ayuda llega completa y sin comisiones. Así de fácil.",

    // FAQ
    "faq-title": "Preguntas frecuentes",
    "faq-q1": "¿Por qué elegir Lupo para enviar dinero?",
    "faq-a1": "Usamos tecnología avanzada para ofrecerte siempre el mejor tipo de cambio ?? y que tu familia reciba más bolivianos. Además, nunca cobramos comisiones: el monto que envías es el monto que reciben.",
    "faq-q2": "¿El dinero llega en bolivianos?",
    "faq-a2": "Sí. El dinero llega a Bolivia al instante y en bolivianos exactos, sin descuentos ni sorpresas ??.",
    "faq-q3": "¿Cómo recibe mi familia el dinero?",
    "faq-a3": "Tú eliges: pueden recibirlo en su cuenta bancaria ?? o recogerlo en efectivo ??, según les convenga más.",
    "faq-q4": "¿Qué tan rápido llega el dinero?",
    "faq-a4": "La entrega es inmediata ?. En minutos tu familia tiene el dinero disponible y te enviamos el comprobante ?.",
    "faq-q5": "¿Hay algún costo o comisión?",
    "faq-a5": "No. En Lupo.lat no cobramos ninguna comisión ? ni cargos ocultos. Todo lo que envías lo reciben en Bolivia.",
    "faq-q6": "¿Es seguro enviar dinero con Lupo?",
    "faq-a6": "Sí. Somos un equipo real, con presencia en Italia y Bolivia ??????????. Acompañamos cada paso de tu envío y te damos transparencia y soporte en todo momento.",
    "faq-q7": "¿Cómo puedo realizar mi primer envío con presencia de un agente de Lupo.lat?",
    "faq-a7": "Es muy sencillo. Solo solicítalo haciendo clic en este <a href='#'><b>enlace ??</b></a> y un agente de nuestro equipo se pondrá en contacto contigo para coordinar el envío de forma personalizada y presencial.",
    "faq-final": "Envío seguro, rápido y sin comisiones. Así de fácil es ayudar a los tuyos con Lupo.lat",

    // CTA y FOOTER
    "cta-title": "¿Listo para enviar más a tu tierra?",
    "cta-subtitle": "No pierdas en comisiones ni tipo de cambio. Con Lupo, tu esfuerzo llega completo y seguro a quienes más quieres.",
    "cta-btn": "Enviar ahora",
    "footer-about": "En Lupo te ayudamos a enviar más dinero a tu familia en Bolivia. Sin comisiones, con el mejor tipo de cambio y atención cercana, siempre.",

    // FOOTER EXTENDIDO
    "footer-servicios": "Servicios",
    "footer-envios": "Envíos a Bolivia",
    "footer-tasas": "Tasas de cambio",
    "footer-calculadora": "Calculadora",
    "footer-ubicaciones": "Ubicaciones",
    "footer-empresa": "Empresa",
    "footer-nosotros": "Sobre nosotros",
    "footer-trabaja": "Trabaja con nosotros",
    "footer-prensa": "Prensa",
    "footer-blog": "Blog",
    "footer-legal": "Legal",
    "footer-terminos": "Términos y condiciones",
    "footer-privacidad": "Política de privacidad",
    "footer-seguridad": "Seguridad",
    "footer-regulaciones": "Regulaciones",
    "footer-contact": "Contacto",
    "footer-mail": "hola@lupo.lat",
    "footer-tel": "+39 334 195 0037",
    "footer-soporte": "Soporte 24/7",
    "footer-whatsapp": "WhatsApp",

    // TESTIMONIOS
    "testimonials-title": "Lo que dicen quienes ya confían en Lupo"
  },
  it: {
    // HERO
    "hero-title-1": "Invia denaro in Bolivia senza commissioni e con il miglior tasso di cambio",
    "hero-title-2": "Invia denaro come se stessi mandando un messaggio di testo, così facile! ??",
    "hero-title-3": "Con LUPO la tua famiglia riceve di più e più velocemente. Risparmia tempo e guadagna tranquillità! ??",
    "hero-title-4": "Tutto dal tuo telefono, ovunque tu sia. Semplice, sicuro e umano.",
    "hero-title-5": "Preferisci un servizio personalizzato? Qui puoi sempre richiederlo. ??",
    "hero-title-6": "Da LUPO siamo più umani, per questo ti diamo il miglior tasso di cambio. ??",
    "hero-title-7": "Ogni invio è un’esperienza piacevole e senza complicazioni.",
    "hero-title-8": "Perché ogni invio conta: qui hai attenzione reale in ogni passo. ??",
    "hero-title-9": "Più che un invio, è un vero sostegno per i tuoi. Fai facile, fai LUPO!",
    "hero-subtitle": "Con Lupo, la tua famiglia riceve fino al 20% in più rispetto ad altri servizi. Aiuta i tuoi cari, senza perdere in commissioni.",
    "btn-send": "Invia ora",
    "btn-how": "Come funziona",

    // CALCULADORA
    "calc-title-1": "Quanto di più può ricevere oggi la tua famiglia?",
    "calc-title-2": "Con Lupo, il tuo denaro vale fino al 20% in più in Bolivia.",
    "calc-title-3": "Invia denaro senza commissioni e aiuta di più la tua terra.",
    "calc-title-4": "Fai arrivare tutto il tuo impegno, senza tagli né sconti.",
    "calc-title-5": "Aiuta la tua famiglia inviando di più, senza pagare commissioni.",
    "calc-title-6": "Scegli Lupo e la tua famiglia riceve di più, velocemente e in modo sicuro.",
    "calc-placeholder": "Importo in EUR",
    "calc-btn": "Invia",

    // FRASES DE AHORRO
    "save-1": "Con LUPO la tua famiglia riceve fino a {{save}} in più in Bolivia. ??",
    "save-2": "Sapevi che qui ricevi {{save}} in più rispetto a Western, Ria o MoneyGram? ??",
    "save-3": "Più guadagni, {{save}} in più di tranquillità. Solo LUPO te lo dà. ??",
    "save-4": "Scegli LUPO: qui il tuo aiuto vale {{save}} extra per i tuoi.",
    "save-5": "Qui ogni euro vale {{save}} in più rispetto alla concorrenza. ??",
    "save-6": "Invia con LUPO e ottieni {{save}} extra ad ogni trasferimento.",
    "save-7": "Invia denaro senza perdere: la tua famiglia riceve {{save}} in più solo qui.",
    "save-8": "Con LUPO il tuo lavoro significa {{save}} in più a casa. ??",

    // BENEFICIOS - Features
    "features-title": "Perché scegliere Lupo?",
    "feature-1-title": "Più per la tua famiglia",
    "feature-1-text": "Con Lupo, ogni euro inviato vale fino al 20% in più in Bolivia, rispetto ai servizi tradizionali.",
    "feature-2-title": "Senza commissioni",
    "feature-2-text": "Non applichiamo nessuna commissione. Tutto quello che mandi arriva completo ai tuoi cari.",
    "feature-3-title": "Consegna immediata",
    "feature-3-text": "Il denaro arriva in pochi minuti in Bolivia, sempre in boliviani esatti e senza sorprese.",
    "feature-4-title": "Massima sicurezza",
    "feature-4-text": "I tuoi invii sono protetti, con tecnologia avanzata e tracciamento in tempo reale.",
    "feature-5-title": "Assistenza personalizzata",
    "feature-5-text": "Siamo qui per aiutarti. Personale in Italia e Bolivia, di persona o online.",
    "feature-6-title": "Facile e senza complicazioni",
    "feature-6-text": "Invia da dove vuoi, senza burocrazia né complicazioni.",

    // COMO FUNCIONA
    "how-title": "Come funziona LUPO?",
    "step-1-title": "Depositi in Italia",
    "step-1-text": "Puoi depositare su conto bancario o di persona con il nostro staff, come preferisci.",
    "step-2-title": "Consegniamo in Bolivia",
    "step-2-text": "Il denaro viene consegnato su conto bancario o in contanti, come preferisce la tua famiglia.",
    "step-3-title": "Tracciamento e ricevute",
    "step-3-text": "Ti avvisiamo quando il denaro è disponibile, con ricevute e assistenza in ogni momento.",
    "how-final": "In pochi minuti il tuo aiuto arriva completo e senza commissioni. Così semplice.",

    // FAQ
    "faq-title": "Domande frequenti",
    "faq-q1": "Perché scegliere Lupo per inviare denaro?",
    "faq-a1": "Usiamo tecnologia avanzata per offrirti sempre il miglior tasso di cambio ?? e la tua famiglia riceve di più. Mai commissioni: quello che invii è quello che ricevono.",
    "faq-q2": "Il denaro arriva in boliviani?",
    "faq-a2": "Sì. Il denaro arriva subito in Bolivia in boliviani esatti, senza sconti né sorprese ??.",
    "faq-q3": "Come riceve i soldi la mia famiglia?",
    "faq-a3": "Scegli tu: possono riceverli su conto bancario ?? o ritirarli in contanti ??, come preferiscono.",
    "faq-q4": "Quanto tempo ci mette il denaro?",
    "faq-a4": "La consegna è immediata ?. In pochi minuti la tua famiglia ha già i soldi e ti inviamo la ricevuta ?.",
    "faq-q5": "Ci sono costi o commissioni?",
    "faq-a5": "No. Su Lupo.lat non ci sono commissioni ? né costi nascosti. Tutto quello che invii arriva in Bolivia.",
    "faq-q6": "È sicuro inviare denaro con Lupo?",
    "faq-a6": "Sì. Siamo un team reale, presente in Italia e Bolivia ??????????. Ti assistiamo in ogni fase e ti garantiamo trasparenza e supporto.",
    "faq-q7": "Come posso fare il mio primo invio con un agente Lupo.lat?",
    "faq-a7": "È molto semplice. Richiedilo cliccando su questo <a href='#'><b>link ??</b></a> e un nostro agente ti contatterà per organizzare un invio personalizzato e di persona.",
    "faq-final": "Invio sicuro, veloce e senza commissioni. Così è facile aiutare chi ami con Lupo.lat",

    // CTA y FOOTER
    "cta-title": "Pronto a inviare di più a casa tua?",
    "cta-subtitle": "Non perdere in commissioni o tasso di cambio. Con Lupo il tuo sforzo arriva completo e sicuro a chi ami.",
    "cta-btn": "Invia ora",
    "footer-about": "Con Lupo aiuti la tua famiglia in Bolivia a ricevere di più. Nessuna commissione, miglior tasso di cambio e assistenza sempre.",

    // FOOTER EXTENDIDO
    "footer-servicios": "Servizi",
    "footer-envios": "Invii in Bolivia",
    "footer-tasas": "Tassi di cambio",
    "footer-calculadora": "Calcolatrice",
    "footer-ubicaciones": "Sedi",
    "footer-empresa": "Azienda",
    "footer-nosotros": "Chi siamo",
    "footer-trabaja": "Lavora con noi",
    "footer-prensa": "Stampa",
    "footer-blog": "Blog",
    "footer-legal": "Legale",
    "footer-terminos": "Termini e condizioni",
    "footer-privacidad": "Privacy",
    "footer-seguridad": "Sicurezza",
    "footer-regulaciones": "Regolamenti",
    "footer-contact": "Contatto",
    "footer-mail": "hola@lupo.lat",
    "footer-tel": "+39 334 195 0037",
    "footer-soporte": "Supporto 24/7",
    "footer-whatsapp": "WhatsApp",

    // TESTIMONIOS
    "testimonials-title": "Cosa dicono di Lupo"
  }
};

const heroPhraseKeys = [
  "hero-title-1","hero-title-2","hero-title-3","hero-title-4","hero-title-5","hero-title-6","hero-title-7","hero-title-8","hero-title-9"
];
const calcPhraseKeys = [
  "calc-title-1","calc-title-2","calc-title-3","calc-title-4","calc-title-5","calc-title-6"
];
const savePhraseKeys = [
  "save-1","save-2","save-3","save-4","save-5","save-6","save-7","save-8"
];
